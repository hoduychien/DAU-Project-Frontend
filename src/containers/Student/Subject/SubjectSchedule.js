...
class SubjectSchedule extends Component {
  ...
  getAllMonth = async () => {
    ...
    if (this.props.subjectId && this.props.subjectId !== -1) {
      ...
      let res = await getScheduleByMonth(
        id,
  ...
  render() {
    let { arrMonth, subjectSchedule, isOpenModalBooking, dataScheduleChoose } =
      this.state;
          ...
          subjectSchedule.map((item, index) => {
            let time =
              language === languages.VI
                ? item.timeTypeData.vi
                : item.timeTypeData.en;
            let date = moment(new Date(parseInt(item.date))).format(
              "DD-MM-YYYY"
            );
            return (
              <div>
                <div className="schedule-item">
                  ...
                  <div className="schedule-date">
                    {language === languages.VI ? "Khai giảng :" : "Opening :"}{" "}
                    {date}
                  </div>
                  <div
                    key={index}
